...
export default function ChantierSelect() {
  ...
  const creer = async (e) => {
    ...
    setLoading(true)
    const { data } = await supabase.from('chantiers').insert({
      nom: form.nom.trim(),
      ...
      <div style={{ width:'100%', maxWidth:500, marginTop:40 }}>
        ...
        {chantiers.length === 0 && !showNew ? (
          ...
          <div style={{ marginBottom:16 }}>
            {chantiers.map(c => (
              <div key={c.id} style={{ background:'var(--surface)', borderRadius:12, padding:'16px 18px', marginBottom:10, boxShadow:'var(--shadow)' }}>
                {editId === c.id ? (
